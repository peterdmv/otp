...
init([Manager, ConfigDB, AcceptTimeout]) ->
	...
	{error, {tls_alert, {_, AlertDesc}} = Error} ->
            ModData = #mod{config_db = ConfigDB, init_data = #init_data{peername = Peername,
                                                                        sockname = Sockname}},
            httpd_util:error_log(ConfigDB, httpd_logger:error_report(tls, AlertDesc, ModData)),
	    exit({shutdown, Error}); 
        {error, closed} = Error ->
            ModData = #mod{config_db = ConfigDB, init_data = #init_data{peername = Peername,
                                                                        sockname = Sockname}},
            httpd_util:error_log(ConfigDB, httpd_logger:error_report(tls, "Connection unexpectedly closed by peer", ModData)),
	    exit({shutdown, Error}); 
        {ok, TLSSocket} ->
...
handle_call(Request, From, #state{mod = #mod{config_db = Db} = ModData} = State) ->
    httpd_util:error_log(Db, 
                         httpd_logger:error_report(internal, "Unexpected request: "
                                                   "~n~p"
                                                   "~nto request handler (~p) from ~p"
                                                   "~n",  [Request, self(), From], ModData)),
...
handle_cast(Msg, #state{mod = #mod{config_db = Db} = ModData} = State) ->
    httpd_util:error_log(Db,
                         httpd_logger:error_report(internal, "Unexpected message: "
                                                   "~n~p"
...
handle_info({Proto, Socket, Data}, 
  ...
  when (((Proto =:= tcp) orelse 
	...
	{error, {size_error, MaxSize, ErrCode, ErrStr}, Version} ->
	    ...
	    httpd_response:send_status(NewModData, ErrCode, ErrStr, "max size is ~p~n", [MaxSize]),
...
handle_http_msg({Method, Uri, Version, {RecordHeaders, Headers}, Body},
	...
	{error, {not_supported, What}} ->
	    httpd_response:send_status(ModData#mod{http_version = Version},
				       501, {Method, Uri, Version}, ": ~p~n", [What]),
	    {stop, normal, State#state{response_sent = true}};
	...
	{error, {bad_request, {malformed_syntax, URI}}} ->
	    httpd_response:send_status(ModData#mod{http_version = Version},
				       400, URI, "Malformed syntax in URI: ~p~n", [URI]),
	    {stop, normal, State#state{response_sent = true}};
	{error, {bad_version, Ver}} ->
	    httpd_response:send_status(ModData#mod{http_version = "HTTP/0.9"}, 400, Ver, "Malformed syntax version: ~p~n", [Ver]),
	    {stop, normal, State#state{response_sent = true}}
    end;
	...
	"chunked" ->
		...
		throw:Error ->
		    httpd_response:send_status(ModData, 400, 
					       "Bad input", "Chunk decoding failed: ~p~n", [Error]),
		    {stop, normal, State#state{response_sent = true}}  
	    end;
	Encoding when is_list(Encoding) ->
	    httpd_response:send_status(ModData, 501, 
				       "Unknown Transfer-Encoding", ": ~p~n", 
                                       [Encoding]),
	    {stop, normal, State#state{response_sent = true}};
	_ -> 
...
error_log(Format, Args,  #mod{config_db = ConfigDB} = ModData) ->
    httpd_util:error_log(ConfigDB, 
                         httpd_logger:error_report(http, "Error reading request: ",
                                                   Format,
                                                   Args, ModData)).
