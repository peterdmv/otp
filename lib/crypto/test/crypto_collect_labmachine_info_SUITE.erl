...
-ifndef(SAVE_PATH_ROOT).
-define(SAVE_PATH_ROOT, "/net/laura/ldisk/hans/crypto_info").
-endif.
...
init_per_suite(Config) ->
    try crypto:start() of
        ...
        _:_ -> ct:fail(no_crypto)
    end,
    
    LocalFile = priv_file(Config, hostname()++".cryptodata"),
    RemoteFile = filename:join(?SAVE_PATH_ROOT, hostname()++".data"),
    ...
    case TransferType of
        ...
        ssh ->
            SCP = "scp "++"laura:"++RemoteFile++" "++remove_drive_letter(LocalFile),
            Result = os:cmd(SCP),
            ct:pal("Use scp: ~s~n-> ~s",[SCP, Result])
    end,
...
end_per_suite(Config) ->
    ...
    case proplists:get_value(transfer_type,Config) of
        ...
        ssh ->
            SCP = "scp "++remove_drive_letter(LocalFile)++" "++"laura:"++RemoteFile,
            Result = os:cmd(SCP),
            ct:pal("Use scp: ~s~n-> ~s",[SCP, Result])
    end,
...
new_entry() ->
    ...
    ++ crypto_info([info_lib,info_fips,supports,ec_curves])
    ++ try_rsa().
...
pk_enc_dec(PadOpts, KeyEnc, Fenc, KeyDec, Fdec) ->
    ...
    lists:sort(
      [LogOpt || Opt <- PadOpts,
                 {LogOpt, Opts} <- gen_logopt_opts(Opt),
                 Ptxt == begin
...
test_sign_verify(Pub, Pri, RsaOpts) ->
    ...
    lists:sort(
      [{LogOpt, OkHashs} || Opt <- RsaOpts,
                            {LogOpt, Opts}  <- gen_logopt_opts(Opt),
                            OkHashs <- [test_sign_verify_hashs(Pub, Pri, Opts, Hashs)],
                            OkHashs =/= []
      ]).
