...
static ERTS_INLINE void db_unlock(DbTable* tb, db_lock_kind_t kind)
{
     ...
     */
    if (kind == NOLCK_ACCESS) {
        return;
    } else if (tb->common.type & DB_FINE_LOCKED) {
	if (kind == LCK_WRITE) {
	    ASSERT(tb->common.is_thread_safe);
                                          ...
                                          Uint* freason_p)
{
    if (tb->common.status & DB_BUSY) {
        ...
        if (continuation_state != NULL) {
            const long iterations_per_red = 32;
            const long reds = iterations_per_red * ERTS_BIF_REDS_LEFT(p);
            long nr_of_reductions =
                IF_DEBUG_OR(reds * erts_sched_local_random_float((Uint)freason_p),
                            reds);
                                       ...
                                       ets_insert_2_list_status on_success_status)
{
    ...
    do {
        ...
        if (tb == NULL) {
            ...
            ASSERT(erts_lc_rwmtx_is_rwlocked(&tb->common.rwlock));
...
static BIF_RETTYPE ets_insert_2_list_driver(Process* p,
                                            ...
                                            Eterm list,
                                            int is_insert_new) {
    const Uint iterations_per_red = 32;
    const long reds = iterations_per_red * ERTS_BIF_REDS_LEFT(p);
    long nr_of_reductions =
        IF_DEBUG_OR(reds * erts_sched_local_random_float((Uint)&p),
                    reds);
    const long init_reds = nr_of_reductions;
